                ...
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="fw-bold">{{ $reservation->total_price }} din</td>
                            <td>
                                @if($reservation->status == 'confirmed')
                                    <span class="badge bg-success">Potvrdeno</span>
                                ...
                                @if($reservation->status == 'confirmed')
                                    <div class="d-flex gap-2">
                                        <form action="{{ route('provider.complete-reservation', $reservation->id) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="btn btn-sm btn-success">Zavrsi</button>
                                        </form>
                                        <form action="{{ route('provider.reject-reservation', $reservation->id) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="btn btn-sm btn-danger"
                                                    onclick="return confirm('Da li ste sigurni da zelite da odbijete ovu rezervaciju?')">
                                                Odbij
                                            </button>
                                        </form>
                                    </div>
                                @elseif($reservation->status == 'completed')
                                    <span class="text-muted small">Zavrseno</span>
                                @elseif($reservation->status == 'rejected')
